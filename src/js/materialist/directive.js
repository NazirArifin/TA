...
app.directive('showModal', ['$http', 'notify', function($http, notify) {
	return function($scope, elm, attrs) {
		...
		// 5 laporkan post ke admin
		// 6 upgrade keanggotaan
		if (attrs.target == 'modal-5' || attrs.target == 'modal-6') {
			$(el).on('click', function(e) {
				showModal();
			});
		}
		
		if (attrs.target == 'modal-7') {
			$(el).on('click', function(e) {
				$('#report-komentar-id').val(attrs.idLapor);
				showModal();
			});
		}
	};
...
/* Laporkan kiriman */
app.directive('reportData', ['$http', 'notify', function($http, notify) {
	return function($scope, elm, attrs) {
		elm.on('click', function(e) {
