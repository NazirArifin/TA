...
class ReviewModel extends ModelBase {
	...
	public function get_review($produk = '', $member = '') {
		...
		if ( ! empty($produk)) {
			...
			$run	= $this->db->query("SELECT a.ID_REVIEWPRODUK, a.ISI_REVIEWPRODUK, a.TANGGAL_REVIEWPRODUK, a.STATUS_REVIEWPRODUK, a.SKOR_REVIEWPRODUK, b.KODE_ANGGOTA, b.NAMA_ANGGOTA, b.FOTO_ANGGOTA, b.VALID_ANGGOTA FROM reviewproduk a, anggota b WHERE a.ID_ANGGOTA = b.ID_ANGGOTA AND a.ID_PRODUK = '$id' AND (STATUS_REVIEWPRODUK = '1' OR STATUS_REVIEWPRODUK = '2')");
            
            if ( ! empty($run)) {
				foreach ($run as $val) {
					// rating
                    $rating = array(
                        'fill'  => intval($val->SKOR_REVIEWPRODUK),
                        'empty' => intval(5 - $val->SKOR_REVIEWPRODUK)
                    );
                    $r[]	= array(
			...
			$id 	= $run->ID_ANGGOTA;
			$r 		= array();
			$run 	= $this->db->query("SELECT b.ID_PRODUK, b.NAMA_PRODUK, b.FOTO_PRODUK, a.ID_REVIEWPRODUK, a.ISI_REVIEWPRODUK, a.TANGGAL_REVIEWPRODUK, a.SKOR_REVIEWPRODUK FROM reviewproduk a, produk b WHERE a.ID_PRODUK = b.ID_PRODUK AND a.ID_ANGGOTA = '$id' AND a.STATUS_REVIEWPRODUK = '2'");
			if ( ! empty($run)) {
				foreach ($run as $val) {
	...
	public function save_review($id = 0) {
		...
		extract($this->prepare_post(array('sender', 'produk', 'data', 'rating')));
		...
		if (empty($run)) {
			$ins	= $this->db->query("INSERT INTO reviewproduk VALUES(0, '$produk', '$id', '$data', '$rating', NOW(), '1')");
			$type	= true;
		} else {
			$now	= time();
			$lpost	= datedb_to_tanggal($run->TANGGAL_REVIEWPRODUK, 'U');
			if (($now - $lpost) < (7 * 24 * 60 * 60)) {
				$type 	= FALSE;
				$rdata	= 'Anda hanya dapat mengirim satu review satu kali dalam seminggu pada produk yang sama';
			} else {
				$ins	= $this->db->query("INSERT INTO reviewproduk VALUES(0, '$produk', '$id', '$data', '$rating', NOW(), '1')");
				$type	= TRUE;
			}
