...
/* admin controller */
app.controller('AdminCtrl', function($scope, $location, $http, notify) {
	if ($scope.token) $scope.me();
	
	$scope.edit = false;
	$scope.setStatus = function(i, s) {
		$http.post($scope.server + '/admin/' + i, { status: s }).
		success(function(d) { $scope.loadData(); }).error(function(e, s, h) {
			alertify.error('Terjadi kesalahan. Periksa koneksi internet Anda');
		});
	};
	...
	$scope.adminList = [];
	$scope.loadData = function() {
		$http.get($scope.server + '/admin?' + jQuery.param($scope.page)).
		success(function(d) {
			$scope.adminList = d.admin;
			$scope.page.numpage = d.numpage;
		}).error(function(e, s, h) {
			alertify.error('Terjadi kesalahan. Periksa koneksi internet Anda');
		});
